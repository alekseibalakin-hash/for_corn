...
// Ёмкость кольца. localStorage НЕ ограничен 4КБ (в отличие от CloudStorage), событие ~150 байт ⇒
// 1500 событий ≈ 225КБ — запас огромный. При обычной игре это недели-месяц, при активной — ≥неделя.
// Цель: «получила приз сегодня → пожаловалась завтра» точно остаётся в окне (с большим запасом).
