...
// ============================================================================
// ФАЗА 2 — резюм потока/слота (§2.3, §2.5 briefs/blocks-phase2.md).
// ============================================================================

describe('makePieceStream(seed, startPos) — резюм продолжает ТОТ ЖЕ поток', () => {
  it('startPos=0 байт-в-байт совпадает с прежним поведением (полный поток)', () => {
    const a = makePieceStream(12345);
    const b = makePieceStream(12345, 0);
    const seqA = [a.nextSet(), a.nextSet(), a.nextSet()];
    const seqB = [b.nextSet(), b.nextSet(), b.nextSet()];
    expect(seqB).toEqual(seqA);
  });

  it('промотанный поток выдаёт ровно «хвост» исходного (резюм с середины)', () => {
    const full = makePieceStream(777);
    full.nextSet(); // 3 фигуры
    full.nextSet(); // ещё 3 → pos = 6
    const tailFull = [full.nextSet(), full.nextSet()];

    const resumed = makePieceStream(777, 6); // продолжаем с позиции 6
    const tailResumed = [resumed.nextSet(), resumed.nextSet()];
    expect(tailResumed).toEqual(tailFull);
    expect(resumed.pos()).toBe(12); // 6 промотано + 6 вытянуто
  });

  it('pos() после промотки = startPos (ещё ни одной фигуры не вытянуто)', () => {
    expect(makePieceStream(1, 9).pos()).toBe(9);
  });
});

describe('isResumableBlocksSlot — слот резюмим ТОЛЬКО если level === bbMaxLevel + 1 (§2.3)', () => {
  const slot = (level: number): BlockLevelState => ({
    level,
    seed: 1,
    setsLeft: 5,
    goal: { kind: 'clearBlocks', target: 2 },
    progress: 0,
    streamPos: 3,
    grid: emptyGrid(),
    currentPieces: [{ cells: [{ r: 0, c: 0 }] }],
  ...
  it('следующий непройденный (level = max+1) → резюмим', () => {
    expect(isResumableBlocksSlot(slot(6), 5)).toBe(true);
  });

  it('УСТАРЕВШИЙ слот (level ≤ глубины — глубина ушла вперёд) → НЕ резюмим (фикс «всегда L25»)', () => {
    expect(isResumableBlocksSlot(slot(5), 5)).toBe(false); // уже пройден
    expect(isResumableBlocksSlot(slot(3), 5)).toBe(false); // давно пройден
  });

  it('слот из будущего (level > max+1 — рассинхрон) → НЕ резюмим', () => {
    expect(isResumableBlocksSlot(slot(8), 5)).toBe(false);
  });

  it('null → НЕ резюмим (нет незаконченного)', () => {
    expect(isResumableBlocksSlot(null, 0)).toBe(false);
  });

  it('новый игрок (глубина 0) резюмит только слот уровня 1', () => {
    expect(isResumableBlocksSlot(slot(1), 0)).toBe(true);
    expect(isResumableBlocksSlot(slot(2), 0)).toBe(false);
  });
});

describe('blocksStateFromLevel — свежий старт/ретрай: полный бюджет, нулевой прогресс, первый набор', () => {
  it('полный setsBudget, progress 0, первый набор вытянут (streamPos=3)', () => {
    const lvl = generateLevel(4, 4242);
    const st = blocksStateFromLevel(lvl);
    expect(st.level).toBe(lvl.level);
    expect(st.seed).toBe(lvl.seed);
    expect(st.setsLeft).toBe(lvl.setsBudget);
    expect(st.progress).toBe(0);
    expect(st.goal).toEqual(lvl.goal);
  ...
  it('первый набор = первый nextSet() того же seed (игрок получает фигуры солвера)', () => {
    const lvl = generateLevel(2, 9001);
    const st = blocksStateFromLevel(lvl);
    const expected = makePieceStream(lvl.seed).nextSet();
    expect(st.currentPieces).toEqual(expected);
  });

  it('детерминизм: тот же level/seed → тот же стартовый снимок', () => {
    const a = blocksStateFromLevel(generateLevel(3, 5));
    const b = blocksStateFromLevel(generateLevel(3, 5));
    expect(a).toEqual(b);
  });

  it('снимок резюмим как следующий уровень (бамп глубины ⇒ level === max+1)', () => {
    // Проигрыш→ретрай: глубина = level-1, ретрай-слот level === (level-1)+1 ⇒ резюмим.
    const lvl = generateLevel(7, 123);
    const st = blocksStateFromLevel(lvl);
    expect(isResumableBlocksSlot(st, lvl.level - 1)).toBe(true);
  });
});
