...
> и честно проходимы живым игроком (casual-коридор). Её код в рабочем дереве (uncommitted, поверх прод v18).
...
Фаза 1 (src/games/blocks/{logic,levels}.ts + тесты + content/blocks.json + depthMirror-фабрика) ПРИНЯТА и
лежит в рабочем дереве (uncommitted, поверх прод v18). Сделай игру играбельной из хаба. ⛔ НИЧЕГО НЕ
